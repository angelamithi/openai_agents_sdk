...
from context import set_context  # ✅ Assuming you have a `context.py` with set_context(thread_id, key, value)

SERP_API_KEY = os.getenv("SERP_API_KEY")  # 🔐 Ensure this is set in your environment

def search_flight(data: SearchFlightInput, thread_id: Optional[str] = None) -> Optional[SearchFlightOutput]:
    ...
    try:
        ...
        # ✅ Store context only if thread_id is available
        if thread_id and flight_results:
            set_context(thread_id, "last_flight_origin", data.origin)
            set_context(thread_id, "last_flight_destination", data.destination)
            set_context(thread_id, "last_number_of_travelers", total_passengers)

            # Set the cost of the first flight option (can be updated later after user selects)
            ...
            if first_price and isinstance(first_price, (int, float, str)) and str(first_price).replace('.', '', 1).isdigit():
                set_context(thread_id, "last_flight_cost", float(first_price))
